...
本期（28日）は、検索表示・クリックは前期比で減った一方、GA4 上のセッションは増えた。ただし **ブログページには GA / GTM タグが無く、ブログのサイト内行動はほぼ未計測**である。検索ではブログ記事（技術検証・ツール比較）が流入の主戦場で、LP（`/`）は表示は少ないが CTR が高い。

コミュニティ目的（興味→参加、参加者のアウトプット、運営の学習）に照らすと、最優先は「アクセス増」ではなく、(1) **ブログ計測の復旧**、(2) **参加フォーム到達・送信の計測**、(3) **ブログ記事の title / description 整備**（検索結果と記事内容の一致）、である。フォーム経由の新規参加者は plan.md 上「現状 0名/年」とあり、本期データでもコンバージョンは **未計測**のため増減は判断できない。
...
| サイト | 対象者 | 提案 KGI | 現状 |
| --- | --- | --- | --- |
| LP | 外部者 | 参加フォーム送信（ユニーク） | **未計測**（フォーム側・GA4キーイベント未確認）。plan.md の「ブログ経由 10名/年」と役割分担する場合は「LP 経由のフォーム送信」を別 KPI に |
...
- サイト内: セッション、LP→フォーム到達、ブログ→LP / フォーム遷移（**計測復旧後**）
...
| フォーム送信未計測 | **高** | キーイベント / コンバージョンを本収集では取得していない。KGI（年10名）をサイトデータから検証不可 |
...
| 参加検討に必要な情報 | 無料・Slack/GitHub 招待・1〜2日で連絡、と明記 | フォーム完了率は未計測 |
| 検索・外部からの獲得 | オーガニックセッション 31（サイト全体）。LP のブランドクエリは弱い（`hagakure` 表示6・クリック1） | Direct 51 は内訳不明（Slack 等の可能性は仮説） |
| 申し込み・関連ページ移動 | `/term_use/` `/voice/` へのセッションは少数。フォーム到達は未計測 | — |
...
- **LP**: 検索からのクリックは少ないが CTR は高い。セッションは主に `/`。参加完了は未計測のため、目的達成は「わからない」。
...
10. フォーム送信数・新規参加者数は本期データでは取得できていない。
...
| フォーム経由参加が 0（plan 記載） | 到達不足、フォーム摩擦、または計測漏れ | CV 未計測のため切り分け不能 | フォーム回答数、到達イベント | フォーム回答の手動確認＋GA4 イベント |
...
2. 参加フォームの回答数（月次）と、可能なら GA4 キーイベント
