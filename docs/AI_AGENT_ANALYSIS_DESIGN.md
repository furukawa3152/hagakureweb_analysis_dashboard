...
flowchart TB
  ...
  H4["⑤ 人: 実行内容を決定<br/>人または承認済みAIが実装・検証"]:::human

  H1 --> H2 --> A1 --> R1 --> A2 --> R2 --> H3 --> H4
  H4 -->|"完了内容をaction-logへ追記し翌月"| H1
```

月次分析としてLLMが自動化する範囲は、収集、検算、分析、原因仮説、
改善アクション案の作成までである。実行の最終判断、公開、現実世界での活動は人が行う。
承認後の実装・検証は月次分析と分離し、人または通常のコーディング依頼を受けたAIが行う。
...
| 実装・検証 | 人が承認し、人またはAIが実行 | 分析実行とは分離する |
| 公開 | 人 | 最終判断は人が行う |
| 実施記録 | 人が確認し、人またはAIが追記 | 完了後だけ追記 |
...
このワークフローが生成・更新し、人が通常見る成果物は `reports/` の2ファイルである。
履歴、月別フォルダ、実行IDは新たに作らず、既存の旧成果物は分析入力に使用しない。
